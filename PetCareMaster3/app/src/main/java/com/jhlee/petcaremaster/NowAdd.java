...
public class NowAdd extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //내부저장소에서 마지막으로 설정한 자동계산 값을 가져옴
        Auto = getAutoPreferences();
        ...
        NowAutoB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(getApplicationContext(), "현재 Auto값은 "+Auto,  Toast.LENGTH_LONG).show();
    ...
    public int getAutoPreferences(){
        SharedPreferences sharedPreferences = getSharedPreferences(Name, 0);
        //자동계산 값이 없으면 0으로 default 설정
        return sharedPreferences.getInt("is_Auto", 0);
    ...
    public void savePreferencesAuto(){
        SharedPreferences sharedPreferences = getSharedPreferences(Name, 0);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        int value = Auto;
